...
app.get("/api/establishments", async (request) => {
    ...
    const { rows } = await pool.query(`SELECT DISTINCT e.id, e.uai, e.name, e.kind
      ...
      ORDER BY e.name`, [user.sub]);
    return rows;
});
app.get("/api/agencies", async (request) => {
    const user = await requireUser(request);
    if (await isPlatformAdmin(user.sub)) {
        const { rows } = await pool.query(`SELECT a.id,a.name FROM accounting_agencies a WHERE a.active=true ORDER BY a.name`);
        return rows;
    }
    const { rows } = await pool.query(`SELECT DISTINCT a.id, a.name FROM accounting_agencies a JOIN user_agency_roles uar ON uar.agency_id=a.id
      WHERE uar.user_id=$1 AND a.active=true ORDER BY a.name`, [user.sub]);
    return rows;
});
app.get("/api/agencies/:id/dashboard", async (request, reply) => {
    const user = await requireUser(request);
    const agencyId = request.params.id;
    const allowed = await pool.query(`SELECT 1 FROM user_agency_roles WHERE user_id=$1 AND agency_id=$2`, [user.sub, agencyId]);
    if (!allowed.rowCount)
        return reply.code(403).send({ error: "FORBIDDEN" });
    const { rows } = await pool.query(`SELECT e.id, e.uai, e.name,
      ...
      GROUP BY e.id, c.id
      ORDER BY e.name`, [agencyId]);
    return rows;
});
app.get("/api/repository-versions", async (request) => {
    await requireUser(request);
    const { rows } = await pool.query(`SELECT rv.id, r.code AS repository_code, r.label AS repository_label,
...
app.get("/api/campaigns/:id", async (request, reply) => {
    ...
    const { rows } = await pool.query(`SELECT c.id, c.establishment_id, e.uai, e.name AS establishment_name,
...
app.get("/api/campaigns", async (request, reply) => {
    const user = await requireUser(request);
    const establishmentId = request.query.establishmentId;
    if (!establishmentId)
        return reply.code(400).send({ error: "ESTABLISHMENT_REQUIRED" });
    const access = await establishmentAccess(user.sub, establishmentId);
    if (!access.canRead)
        return reply.code(403).send({ error: "FORBIDDEN" });
    const { rows } = await pool.query(`SELECT c.*, rv.version AS repository_version,
           (SELECT count(*)::int FROM questions q2 WHERE q2.repository_version_id=c.repository_version_id AND q2.active=true) AS question_count
       FROM campaigns c JOIN repository_versions rv ON rv.id=c.repository_version_id
      WHERE c.establishment_id=$1 AND (NOT $3::boolean OR EXISTS(SELECT 1 FROM audit_missions m WHERE m.auditor_user_id=$2 AND m.campaign_id=c.id AND m.status IN('PREPARED','OPEN') AND CURRENT_DATE BETWEEN m.valid_from AND m.valid_until)) ORDER BY c.created_at DESC`, [establishmentId, user.sub, access.roles.includes("AUDITOR") && !access.directRoles.some((r) => r !== "AUDITOR") && !access.agencyRoles.length]);
    return rows;
});
app.post("/api/campaigns", async (request, reply) => {
    ...
    if (!access.canWrite)
        return reply.code(403).send({ error: "FORBIDDEN" });
    const { rows } = await pool.query(`INSERT INTO campaigns(establishment_id,repository_version_id,label,status,created_by)
     VALUES($1,$2,$3,'DRAFT',$4) RETURNING *`, [parsed.data.establishmentId, parsed.data.repositoryVersionId, parsed.data.label, user.sub]);
...
app.get("/api/campaigns/:id/questions", async (request, reply) => {
    ...
    if (!access?.canRead)
        return reply.code(403).send({ error: "FORBIDDEN" });
    const { rows } = await pool.query(`SELECT q.id, q.code, q.domain, q.label, q.responsibility, q.weight, q.stars, q.badge,
            COALESCE(jsonb_agg(
              jsonb_build_object(
      ...
      WHERE c.id=$1
      GROUP BY q.id
      ORDER BY q.sort_order, q.code`, [campaignId]);
    return rows;
});
app.put("/api/campaigns/:campaignId/answers/:questionId", async (request, reply) => {
    ...
    if (["VALIDATED", "ARCHIVED"].includes(campaign.rows[0].status))
        return reply.code(409).send({ error: "CAMPAIGN_READ_ONLY" });
    const access = await campaignAccess(user.sub, campaignId);
    if (!access?.canWrite)
        return reply.code(403).send({ error: "FORBIDDEN" });
    ...
    try {
        const result = await tx(async (client) => {
            const current = await client.query(`SELECT * FROM answers WHERE campaign_id=$1 AND question_id=$2 AND sphere=$3 FOR UPDATE`, [campaignId, questionId, parsed.data.sphere]);
            ...
            const saved = old
                ? await client.query(`UPDATE answers
                SET value=$1, comment=$2, version=version+1, updated_by=$3, updated_at=now()
              WHERE id=$4 RETURNING *`, [parsed.data.value, parsed.data.comment, user.sub, old.id])
                : await client.query(`INSERT INTO answers(campaign_id,question_id,sphere,value,comment,version,updated_by)
             VALUES($1,$2,$3,$4,$5,1,$6) RETURNING *`, [campaignId, questionId, parsed.data.sphere, parsed.data.value, parsed.data.comment, user.sub]);
            await client.query(`INSERT INTO audit_events(user_id,establishment_id,module,entity_type,entity_id,operation,before_data,after_data)
        ...
        if (e.statusCode === 409)
            return reply.code(409).send({ error: "CONFLICT", current: e.current ?? null });
        throw e;
    }
});
