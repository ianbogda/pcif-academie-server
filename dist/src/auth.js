...
export async function registerAuth(app) {
    app.post("/api/auth/login", async (request, reply) => {
        ...
        const { rows } = await pool.query(`SELECT id, email, display_name, password_hash, active
       FROM users WHERE lower(email)=lower($1) AND deleted_at IS NULL`, [body.email]);
        const user = rows[0];
        if (!user || !user.active || !(await bcrypt.compare(body.password, user.password_hash))) {
            return reply.code(401).send({ error: "INVALID_CREDENTIALS" });
        }
        const token = app.jwt.sign({ sub: user.id, email: user.email, displayName: user.display_name }, { expiresIn: "8h" });
        ...
        if (user) {
            ...
            if (!recent.rowCount) {
                try {
                    ...
                    await sendPasswordLink(user.email, user.display_name, token, "RESET");
                }
                catch (e) {
