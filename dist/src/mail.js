...
function required(name) { const value = process.env[name]?.trim(); if (!value)
    throw new Error(`Configuration email manquante : ${name}`); return value; }
function transport() {
...
export async function issuePasswordToken(userId, purpose) {
...
export async function sendPasswordLink(to, name, token, purpose) {
