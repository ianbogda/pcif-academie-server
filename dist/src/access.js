...
const WRITE_ROLES = ["PLATFORM_ADMIN", "AGENCY_ACCOUNTANT", "AGENCY_DEPUTY", "HEAD", "SECRETARY_GENERAL", "CONTRIBUTOR"];
const ORDONNATEUR_ROLES = ["PLATFORM_ADMIN", "HEAD", "SECRETARY_GENERAL", "CONTRIBUTOR"];
const COMPTABLE_ROLES = ["PLATFORM_ADMIN", "AGENCY_ACCOUNTANT", "AGENCY_DEPUTY"];
...
export async function establishmentAccess(userId, establishmentId) {
    const admin = await isPlatformAdmin(userId);
    const direct = await pool.query(`SELECT r.code FROM user_establishment_roles uer JOIN roles r ON r.id=uer.role_id
    ...
    const agency = await pool.query(`SELECT r.code FROM user_agency_roles uar JOIN roles r ON r.id=uar.role_id
    ...
    const directRoles = direct.rows.map(r => r.code);
    const agencyRoles = agency.rows.map(r => r.code);
    ...
    const roles = [...new Set([...directRoles, ...agencyRoles, ...(mission.rowCount ? ["AUDITOR"] : []), ...(admin ? ["PLATFORM_ADMIN"] : [])])];
    return {
...
export async function campaignAccess(userId, campaignId) {
    ...
    const hasBusinessAccess = base.isPlatformAdmin || base.directRoles.some((r) => r !== "AUDITOR") || base.agencyRoles.length > 0;
...
export async function canGrantAuditMission(userId, campaignId) {
...
export function canWriteSphere(access, sphere) {
