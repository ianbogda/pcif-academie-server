...
try {
 ...
 `)).rows[0];
    if (!rv)
        throw new Error("Référentiel PCIF-267-2026.09 absent. Exécuter npm run db:reference avant db:demo-sync.");
    ...
    if (count !== 267)
        throw new Error(`Référentiel PCIF incomplet : ${count} questions au lieu de 267.`);
    const admin = (await client.query(`SELECT id FROM users WHERE email='admin@example.test' AND deleted_at IS NULL`)).rows[0];
    if (!admin) {
        console.log("Aucun compte de démonstration : synchronisation ignorée.");
   ...
   RETURNING id`, [auditHash])).rows[0];
    ...
    if (demo.rowCount !== 8)
        console.warn(`Attention : ${demo.rowCount}/8 établissements de démonstration trouvés.`);
    const brossolette = demo.rows.find((e) => e.uai === "0280002B");
    if (brossolette) {
    ...
    for (const e of demo.rows) {
     ...
     ORDER BY c.created_at`, [e.id]);
        let cid;
        const canonical = campaigns.rows.find((c) => c.repository_version_id === rv.id);
        if (canonical) {
            cid = canonical.id;
     ...
     WHERE establishment_id=$1 AND id<>$2
       ...
       AND repository_version_id<>$3`, [e.id, cid, rv.id]);
        ...
        for (const h of history) {
            ...
            if (!historic)
                historic = (await client.query(`INSERT INTO campaigns(establishment_id,repository_version_id,label,status,created_by,created_at,updated_at)
       VALUES($1,$2,$3,'VALIDATED',$4,$5::date,$5::date) RETURNING id`, [e.id, rv.id, h.label, admin.id, h.date])).rows[0];
            else
                await client.query(`UPDATE campaigns SET repository_version_id=$1,status='VALIDATED',created_at=$2::date,updated_at=$2::date WHERE id=$3`, [rv.id, h.date, historic.id]);
         ...
         VALUES($1,$2,$3,$4,$5,1,$6,$7::date)`, [historic.id, q.id, sphere, value, `Diagnostic fictif ${h.label}`, admin.id, h.date]);
            ...
            const actionQuestions = hqs.filter((_, qi) => { const mod = qi % 10; return mod >= h.yes; }).slice(0, 6);
         ...
         VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9::date,$10,$11,$11,$12::date,$12::date)`, [historic.id, q.id, q.responsibility === 'COMPTABLE' ? 'COMPTABLE' : 'ORDONNATEUR', `Action de sécurisation ${ai + 1} — ${h.label}`, ai < 2 ? 'P1' : 'P2', older ? '2024-2025' : '2025-2026', ai % 2 ? 'Secrétaire général' : 'Agent comptable', status, older ? '2025-06-30' : '2026-06-30', status === 'REALISEE' ? 'Action réalisée et preuve archivée.' : 'Action à poursuivre dans la campagne suivante.', admin.id, h.date]);
            }
        }
    }
   ...
   ORDER BY c.created_at LIMIT 1`, [rv.id])).rows[0];
    if (target) {
     ...
     SELECT id,responsibility,sort_order FROM (
       ...
       ON CONFLICT(campaign_id,question_id,sphere) DO NOTHING`, [target.id, q.id, sphere, value, 'Réponse de démonstration sur le référentiel PCIF 267', admin.id]);
            i++;
        }
    }
