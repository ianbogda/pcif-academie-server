...
try {
    ...
    if (!repo)
        repo = (await client.query(`INSERT INTO repositories(code,label) VALUES('PCIF-ACADEMIE','PCIF Académie') RETURNING id`)).rows[0];
    ...
    if (!rv)
        rv = (await client.query(`INSERT INTO repository_versions(repository_id,version,published_at,active)
   VALUES($1,'PCIF-267-2026.09',CURRENT_DATE,true) RETURNING id`, [repo.id])).rows[0];
    ...
    for (const q of ref.questions) {
     ...
     ON CONFLICT(repository_version_id,code) DO UPDATE SET
       ...
       corrective_evaluations=excluded.corrective_evaluations`, [rv.id, q.code, q.domain, q.label, q.responsibility, q.stars, q.weight, q.order, q.category, q.risk,
