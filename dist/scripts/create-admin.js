...
if (args.includes("--initial")) {
    const existing = await pool.query(`SELECT 1 FROM users WHERE is_platform_admin=true AND active=true AND deleted_at IS NULL LIMIT 1`);
...
const { rows } = await pool.query(`INSERT INTO users(email,display_name,password_hash,active,is_platform_admin,deleted_at)
   ...
   RETURNING id,email,display_name,is_platform_admin`, [parsed.data.email, parsed.data.name, hash]);
