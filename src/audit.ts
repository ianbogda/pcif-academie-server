...
export async function registerAudit(app:FastifyInstance){
 ...
 app.post("/api/campaigns/:id/audit/observations",async(request,reply)=>{
  const user=await requireUser(request),id=(request.params as any).id,ctx=await context(user.sub,id);
  if(!ctx||!ctx.auditor||!ctx.observationsAllowed)return reply.code(403).send({error:"AUDIT_OBSERVATION_FORBIDDEN"});
  ...
  if(!p.success)return reply.code(400).send({error:"INVALID_OBSERVATION"});
  ...
  await pool.query(`INSERT INTO audit_events(user_id,establishment_id,module,entity_type,entity_id,operation,after_data) VALUES($1,$2,'AUDIT','OBSERVATION',$3,'CREATE',$4::jsonb)`,[user.sub,ctx.campaign.establishment_id,rows[0].id,JSON.stringify(rows[0])]);return reply.code(201).send(rows[0]);
 });

 app.patch("/api/campaigns/:campaignId/audit/observations/:observationId",async(request,reply)=>{
  const user=await requireUser(request),{campaignId,observationId}=request.params as any,ctx=await context(user.sub,campaignId);if(!ctx)return reply.code(403).send({error:"FORBIDDEN"});
  if(ctx.auditor&&!ctx.access.canWrite)return reply.code(403).send({error:"ESTABLISHMENT_RESPONSE_REQUIRED"});
  const p=z.object({status:z.enum(["ACKNOWLEDGED","CLOSED"]),response:z.string().trim().min(2).max(10000)}).safeParse(request.body);if(!p.success)return reply.code(400).send({error:"INVALID_RESPONSE"});
  const {rows}=await pool.query(`UPDATE audit_observations SET status=$1,response=$2,responded_by=$3,responded_at=now(),updated_at=now() WHERE id=$4 AND campaign_id=$5 RETURNING *`,[p.data.status,p.data.response,user.sub,observationId,campaignId]);return rows[0]||reply.code(404).send({error:"NOT_FOUND"});
 });
}
