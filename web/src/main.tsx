...
function AdminEstablishments(){
 const[items,setItems]=useState<any[]>([]),[editing,setEditing]=useState<any|null>(null),[creating,setCreating]=useState(false);async function load(){setItems(await api.adminEstablishments())}useEffect(()=>{load()},[]);
 return <><div className="hero"><div><small>Administration</small><h1>Établissements</h1><p>Créer et maintenir les EPLE disponibles dans PCIF Académie.</p></div><button className="primary" onClick={()=>setCreating(true)}>+ Nouvel établissement</button></div><section className="panel"><table className="users"><thead><tr><th>UAI</th><th>Établissement</th><th>Type</th><th>Agence comptable</th><th>Statut</th><th></th></tr></thead><tbody>{items.map(e=><tr key={e.id}><td>{e.uai}</td><td><b>{e.name}</b></td><td>{e.kind||"—"}</td><td>{e.agency_name||"Non rattaché"}</td><td><span className={e.active?"ok":"off"}>{e.active?"Actif":"Suspendu"}</span></td><td><button onClick={()=>setEditing(e)}>Modifier</button></td></tr>)}</tbody></table></section>{(creating||editing)&&<EstablishmentModal item={editing} close={()=>{setCreating(false);setEditing(null)}} saved={async()=>{setCreating(false);setEditing(null);await load()}}/>}</>
}
...
function AdminAgencies(){const[agencies,setAgencies]=useState<any[]>([]),[ets,setEts]=useState<any[]>([]),[editing,setEditing]=useState<any|null>(null),[creating,setCreating]=useState(false);async function load(){setAgencies(await api.adminAgencies());setEts(await api.adminEstablishments())}useEffect(()=>{load()},[]);return <><div className="hero"><div><small>Administration</small><h1>Agences comptables</h1><p>Définir l’établissement support et les EPLE rattachés à chaque agence.</p></div><button className="primary" onClick={()=>setCreating(true)}>+ Nouvelle agence</button></div><div className="agency-grid">{agencies.map(a=><article className="agency-card" key={a.id}><div><span className={a.active?"ok":"off"}>{a.active?"Active":"Inactive"}</span><h3>{a.name}</h3><p>Support : <b>{a.support_name}</b> · {a.support_uai}</p></div><strong>{a.establishments.length}<small> EPLE</small></strong><div className="agency-members">{a.establishments.map((e:any)=><span key={e.id}>{e.uai} · {e.name}</span>)}</div><button onClick={()=>setEditing(a)}>Configurer</button></article>)}</div>{(creating||editing)&&<AgencyModal item={editing} establishments={ets} close={()=>{setCreating(false);setEditing(null)}} saved={async()=>{setCreating(false);setEditing(null);await load()}}/>}</>}
