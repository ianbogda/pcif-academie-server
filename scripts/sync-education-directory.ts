...
async function json(url:URL){const response=await fetch(url,{headers:{"user-agent":"PCIF-Academie/0.20 (+https://pcif.eple-tools.fr)"}});if(!response.ok)throw new Error(`Annuaire Éducation indisponible (${response.status})`);return response.json() as Promise<any>}

const academyUrl=new URL(endpoint);academyUrl.searchParams.set("select","code_academie");academyUrl.searchParams.set("where",base);academyUrl.searchParams.set("group_by","code_academie");academyUrl.searchParams.set("limit","100");
const academies=(await json(academyUrl)).results.map((x:any)=>String(x.code_academie)).filter(Boolean);
let total=0;
for(const academy of academies){let offset=0;for(;;){
 const url=new URL(endpoint);url.searchParams.set("where",`${base} AND code_academie='${academy.replace(/'/g,"''")}'`);url.searchParams.set("limit","100");url.searchParams.set("offset",String(offset));
 const body=await json(url),records=body.results||[];
 for(const r of records){const uai=String(r.identifiant_de_l_etablissement||"").toUpperCase();if(!/^[0-9A-Z]{8}$/.test(uai))continue;
