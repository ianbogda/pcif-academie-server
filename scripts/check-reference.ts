...
try{
 ...
 if(!rv) throw new Error("Référentiel PCIF-267-2026.09 absent.");
 ...
 if(count!==267) throw new Error(`Référentiel réglementaire invalide : ${count}/267 questions.`);
 ...
 if(wrong.rowCount){
   ...
   for(const c of wrong.rows) console.warn(`- ${c.uai} · ${c.label} · ${c.version} · ${c.question_count} questions`);
